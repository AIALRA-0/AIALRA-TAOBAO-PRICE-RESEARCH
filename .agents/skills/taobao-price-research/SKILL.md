...
description: "通过淘宝页面和淘宝开放平台只读接口批量搜索并核验淘宝与天猫的当前商品价格、目标规格、店铺、宣传图、评价信号、库存、优惠条件和购买风险，并根据用户约束找出最低价可行商品；用于淘宝查价、最低价、硬件比价、商品链接核验和购买建议请求；不用于自动下单、联系卖家、绕过登录验证、采集账号凭据或比较淘宝以外的平台"
---
...
flowchart LR
    ...
    B --> C["批量读取淘宝搜索结果"]
    C -. "页面不可读" .-> D["调用淘宝官方候选接口"]
    C --> E["去重并排除明显错误商品"]
    D --> E
    E --> F["读取入围商品详情与评价信号"]
    F -. "详情页不可读" .-> G["调用淘宝官方详情接口"]
    F --> H["计算可比成本与风险"]
    G --> H
    H --> I["输出最低价可行商品和覆盖缺口"]
```
...
| `collect-candidates-api` | 浏览器搜索失败后调用淘宝客物料搜索升级版发现候选 | script |
| `prepare-shortlist` | 规范链接、去重、排除明显错误结果并生成详情候选 | script |
| `inspect-details` | 打开入围商品页并核验目标 SKU、店铺、图片、评价和成本 | browser-dom |
| `inspect-details-api` | 详情页失败后调用淘宝客商品详情升级版核验 SKU 价格和库存 | script |
| `rank-offers` | 计算已知总额、风险分、可行性和最终排名 | script |

浏览器搜索失败时，Runner 进入 `collect-candidates-api`

官方候选接口也失败时，Runner 进入 `manual-handoff`

浏览器详情读取失败时，Runner 进入 `inspect-details-api`

官方详情接口也失败时，Runner 进入 `limited-evidence`

两个回退节点只能交付覆盖缺口或人工入口，不能把未经详情核验的搜索卡片宣布为最低价赢家

## 官方接口节点怎样执行

先读取 [淘宝官方接口接入](references/official-api.md)

官方接口只读取商品数据，不控制淘宝账号

真实凭据只放在本机环境变量或系统密钥存储

不要把 `App Key`、`App Secret` 或推广位编号写进入口 JSON、运行产物、日志、学习记录或仓库

淘宝客接口只覆盖可推广商品，任何结果都必须保留这个覆盖缺口

官方接口没有提供的评价、退货、保修、店铺评分和费用字段保持 `unknown`
...
没有淘宝消费者商品 API 时，使用用户已经登录或可以亲自登录的浏览器
...
Runner 会进入声明的官方接口回退节点

不要切换到规避策略的浏览器、脚本或数据来源
