...
description: "批量搜索并核验淘宝与天猫的当前商品价格、目标规格、店铺、宣传图、评价信号、库存、优惠条件和购买风险，并根据用户约束找出最低价可行商品；用于淘宝查价、最低价、硬件比价、商品链接核验和购买建议请求；不用于自动下单、联系卖家、绕过登录验证、采集账号凭据或比较淘宝以外的平台"
---
...
搜索读取失败时，Runner 进入 `manual-handoff`

详情读取失败时，Runner 进入 `limited-evidence`
